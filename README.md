...
Esse arquivo `docker-compose.yml` define uma aplicação composta por quatro serviços: `postgres`, `pgadmin4`, `backend` e `frontend`, e cria uma rede chamada `mynetwork`:
...
* `image: dpage/pgadmin4`: Esse serviço utiliza a imagem oficial do Pgadmin4 disponível no Docker Hub.
* `environment`: Define variáveis de ambiente para configurar o usuário e senha do pgadmin4, como email/nome de usuário (`PGADMIN_DEFAULT_EMAIL`) e senha (`PGADMIN_DEFAULT_PASSWORD`).
* `ports`: Mapeia a porta `16543` do sistema hospedeiro para a porta `80` do contêiner, permitindo que o serviço seja acessado através da porta `16543`.
...
Quando você executa `docker-compose up`, o Docker Compose lerá o arquivo `docker-compose.yml`, criará os serviços conforme as definições especificadas e os iniciará. Isso significa que os contêineres para o banco de dados PostgreSQL, o Pgadmin4, o backend e o frontend serão criados e conectados à rede `mynetwork`. O banco de dados será configurado com os detalhes fornecidos (nome do banco de dados, usuário e senha), e as imagens para os serviços de backend e frontend serão construídas a partir dos Dockerfiles fornecidos. Uma vez iniciados, você poderá acessar o Pgadmin4 através de `http://localhost:16543`, o backend através de `http://localhost:8000` e o frontend através de `http://localhost:8501`. Os dados do banco de dados serão persistidos no volume `postgres_data`.
...
4) Criar a Base do ORM (o Model vai herdar ele)
...
Detalhe que criamos schemas diferentes para os retornos da API. Isso é uma boa prática, pois permite que tenhamos mais flexibilidade para alterar os schemas no futuro.
...
Temos o schema `ProductResponse`, que é o schema que é retornado pela API. Ele é uma classe que herda do schema `ProductBase`, e possui dois campos a mais, que é o id e o created_at. Esses campos são gerados pelo banco de dados.
...
Conseguimos também retornar parâmetros pelo path, no caso do delete, por exemplo, precisamos passar o id do produto que queremos deletar. Para isso, utilizamos o path /products/{product_id}, e definimos o parâmetro product_id na função delete_product.
...
O frontend vai consumir a API, e vai ser responsável por fazer o cadastro, alteração e remoção de produtos.
...
Uma task é uma instância de uma task definition. Ou seja, é uma execução da sua aplicação. Por exemplo, se você tem uma task definition que define que a sua aplicação vai ser executada com 2 instâncias, você terá 2 tasks executando a sua aplicação. Aplicado ao Airflow que vimos no Workshop 02, podemos subir mais de uma instância do Airflow, para garantir que a aplicação vai estar sempre disponível. Além disso, podemos configurar para subir mais instâncias quando a CPU estiver alta, por exemplo.
...
## Novos itens
Este projeto é um fork da aula final do bootcamp de python da Jornada de Dados. Os seguintes itens foram adicionados/modificados
- Adicionado Pgadmin4
- Adicionado dotenv para o banco de dados
